 ...
 */
export const handler = async (event: DbMigrateEvent, context: Context): Promise<DbMigrateResponse> => {
  ...
  try {
    ...
    const migrateOutput = await runMigrate(connectionString, dbMigrationsBucket, migrateCommand);
...
const isDatabaseSecret = (o: any): o is DatabaseSecret => {
  return (
    'username' in o && 'password' in o && 'host' in o && 'engine' in o && 'dbInstanceIdentifier' in o && 'port' in o
  );
};

const buildConnectionString = (dbName: string, databaseSecret: DatabaseSecret): string => {
...
function runMigrate(connectionString: string, bucketName: string, migrateCommand: string): Promise<string> {
