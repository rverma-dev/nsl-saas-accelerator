...
export class DbMigrate extends Construct implements ec2.IConnectable, iam.IGrantable {
  ...
  constructor(scope: Construct, id: string, props: DbMigrateProps) {
    ...
    const code = lambda.DockerImageCode.fromImageAsset(`${__dirname}/function`, {
    ...
    const migrationsDeployment = new s3_deployment.BucketDeployment(this, 'MigrationDeployment', {
    ...
    const s3AssetHash = (migrationsDeployment.node.findChild('Asset1') as s3_assets.Asset).assetHash;
    ...
    const sdkCall: custom.AwsSdkCall = {
      ...
      physicalResourceId: custom.PhysicalResourceId.of(`${id}-AwsSdkCall-${this.onEventHandler.currentVersion.version}`)
    };
    ...
    props.targetDatabaseInstance.connections.allowFrom(this.onEventHandler, ec2.Port.tcp(Token.asNumber(dbPort)));
  ...
  getSecretId(props: DbMigrateProps): secrets_manager.ISecret {
    ...
    if (!secret) {
      throw new Error('No database credentials found in either props or on the target database');
    }
